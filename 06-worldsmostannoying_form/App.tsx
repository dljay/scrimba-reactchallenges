...
type PrivacyResponse = "yes" | "no" | "absolutely"
...
type FormData = {
	...
	rating: string
	marketingResponse: boolean
	wantsToSubmit: boolean
}
...
export default function App() {
	...
	function handleChange(
		...
		let dataToRecord: string | boolean

		if (name === "firstName" || name === "lastName" || name === "email") {
			dataToRecord = hijackResults()
		} else if (type == "radio") {
			dataToRecord = switcharoo()
		} else {
	...
	function fakeSubmit(e: React.SyntheticEvent) {
		e.preventDefault()
		setFormData((prev) => ({
			...formData,
...
/* Challenge
	...
	Note (TypeScript): formData state is declared at the top of App() because TypeScript
	requires variables to be declared before they are referenced in functions above the JSX.
*/

	console.log("JJ-formData:", formData)
	...
	return (
		<form onSubmit={fakeSubmit}>
			...
			<fieldset className="rating-container">
				<legend>
					On a scale of 1-10, with 1 being the worst and 10 being the best, how would you
					rate this form?
				</legend>
