...
interface MathProblem {
    string: string
    answer: string
    submittedResponse: number | undefined
}

export default function App() {
    
    const [mathProblem, setMathProblem] = React.useState<MathProblem>(getRandomProblem)
    ...
    function getRandomProblem(): MathProblem {
    ...
    function getCorrectAnswer(firstNum: number, secondNum: number, operator: Operator): number {
        const x = firstNum
        const y = secondNum
        if (operator === "+") {
            return x + y
    ...
    function getAnswerStatus(num: number) {
    ...
    function getRandomNumber(maximum: number) {
    ...
    function showStates() {
        ...
        const states: Record<string, string | boolean | MathProblem> = {
            mathProblem, 
            currentResponse, 
            recentCorrectAnswer, 
            recentStatusChange, 
        ...
        for (const state in states) {
            ...
            const value = states[state] === "" ? `""` 
                : typeof states[state] === "object" ? stringify(states[state] as MathProblem) 
                : states[state]
            
            function stringify(obj: MathProblem) {
